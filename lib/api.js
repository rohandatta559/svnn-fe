...
export function submitTestimonial({ name, city, quote, website }) {
  return request("/testimonials", {
    method: "POST",
    body: JSON.stringify({ name, city, quote, website }),
  });
}

export function adminGetTestimonials(token) {
  return request("/testimonials/admin", {
    headers: { Authorization: `Bearer ${token}` },
  });
}

export function adminSetTestimonialApproval(token, id, isApproved) {
  return request(`/testimonials/${id}/approval`, {
    method: "PUT",
